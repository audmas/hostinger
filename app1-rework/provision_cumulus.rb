Vagrant.configure("2") do |config|
    ...
    config.vm.provision :shell, inline: "apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 93C4A3FD7BB9C367 && apt-get update -y && apt-get install ansible -y && apt-get install ruby -y && apt-get install bundler -y"
    config.vm.provision :shell, inline: 'sed -i s/bgpd=no/bgpd=yes/ /etc/frr/daemons'
    config.vm.provision :shell, inline: 'systemctl restart frr.service'
